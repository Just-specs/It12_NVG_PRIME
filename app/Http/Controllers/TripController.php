...
class TripController extends Controller
{
    ...
    {
        // Enhanced logging for debugging Railway issues
        \Log::info('Trip assignment attempt', [
            'user_id' => auth()->id(),
            'all_request_data' => $request->all(),
            'request_data_except_token' => $request->except(['_token']),
            'has_delivery_request_id' => $request->has('delivery_request_id'),
            'delivery_request_id_value' => $request->input('delivery_request_id'),
        ...
        try {
            // Check if delivery_request_id is missing and log it
            if (!$request->has('delivery_request_id') || empty($request->input('delivery_request_id'))) {
                \Log::error('delivery_request_id is missing or empty', [
...
class TripController extends Controller
{
    ...
    public function store(Request $request, $deliveryRequestId = null)
    {
...
POST ?? 'No POST data',
                ...
                return redirect()
                    ->back()
                    ->withInput()
                    ->with('error', 'Delivery request ID is missing. Please try again or contact support.');
            }
