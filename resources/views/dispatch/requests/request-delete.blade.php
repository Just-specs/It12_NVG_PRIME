...
@section('title', 'Request Delivery Request Deletion')
...
<div class="container mx-auto px-4 max-w-2xl">
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <div class="bg-orange-50 border-l-4 border-orange-500 p-4 mb-6 rounded">
            <h3 class="font-semibold text-orange-900 mb-2">Delivery Request to be Deleted:</h3>
            <div class="text-orange-800">
                <p><strong>ATW Reference:</strong> {{ $deliveryRequest->atw_reference }}</p>
                <p><strong>Client:</strong> {{ $deliveryRequest->client->name }}</p>
                <p><strong>Status:</strong> {{ ucfirst($deliveryRequest->status) }}</p>
                <p><strong>Pickup:</strong> {{ $deliveryRequest->pickup_location }}</p>
                <p><strong>Delivery:</strong> {{ $deliveryRequest->delivery_location }}</p>
            </div>
        </div>
        ...
        <!-- Request Form -->
        <form method="POST" action="{{ route('requests.submitDeleteRequest', $deliveryRequest) }}">
            @csrf
            ...
            <div class="mb-6">
                ...
                <textarea 
                    ...
                    placeholder="Explain why this delivery request needs to be deleted (minimum 10 characters)...">{{ old('reason') }}</textarea>
